	...
	var Public = {
		...
		buildModalContent: function buildModalContent(response) {
			...
			for (var i = 0; i < response.length; i++) {
				...
				var className = i % 3 == 0 ? 'no-border' : '';
				promoContent += '<div class="promo ' + className + '">' + '<div class="title">' + response[i].NomeTarifa + '<i class="icon tooltip-modal" data-desc="' + response[i].RegrasTarifa + '">?</i></div>' + '<ul>' + descriptionContent + '</ul>' + '<div class="values">' + '<div class="old-value">' + Public.maskPrice(response[i].ValorTarifaSemDesconto) + '</div>' + '<div class="new-value">' + Public.maskPrice(response[i].ValorTarifa) + '</div>' + '</div>' + '<div><button class="btn">Reservar</button></div>' + '</div>';
			}
